...
API_TOKEN = os.getenv("CF_AI_GATEWAY_TOKEN") 
...
def main():
    ...
    # Establish the unified OpenAI-compatible endpoint for Workers AI
    BASE_URL = f"https://gateway.ai.cloudflare.com/v1/{ACCOUNT_ID}/{GATEWAY_NAME}/workers-ai/chat/completions"
    ...
    try:
        response = client.chat.completions.create(
            model=AI_MODEL,
            messages=messages,
            response_format={"type": "json_schema", "json_schema": {"name": "hardware_diagnostic", "schema": response_schema}},
            temperature=0.2,
        )
        
        # The OpenAI SDK handles the payload extraction, we just parse the resulting string
